function mobile_robot_NN_perpendicular()
clc
close all
clear all
%% =========== Set the paramters =======
...
y(k-1)=-.6; % initilize the state y
theta(k-1)=pi/2; % initilize the state theta
tfinal=10; % final simulation time
t=0; % intilize the time 
[draw_x, draw_y]=meshgrid(-1:.1:1, -0.8:.1:1);
draw_u=zeros(19, 21);draw_v=ones(19, 21);
%=====================================
%% =========== The main loop ==========
while(t<=tfinal) 
t=t+T; % increase the time
V=0.5;
quiver(draw_x, draw_y, draw_u, draw_v, 0.5);
hold on
plot(polyshape([-1 -1 1 1],[-1 -0.8 -0.8 -1]))
hold on
theta(k)=theta(k-1); % calculating theta
