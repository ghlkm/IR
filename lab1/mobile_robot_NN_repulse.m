function mobile_robot_NN_repulse()
clc
close all
clear all
%% =========== Set the paramters =======
...
x(k-1)=0.1; % initilize the state x
y(k-1)=0.1; % initilize the state y
theta(k-1)=pi/4; % initilize the state theta
tfinal=100; % final simulation time
t=0; % intilize the time 
[draw_x, draw_y]=meshgrid(-1:.1:1, -1:.1:1);
draw_u=draw_x;draw_v=draw_y;
%=====================================
%% =========== The main loop ==========
while(t<=tfinal) 
t=t+T; % increase the time
V=(x(k-1)^2+y(k-1)^2)^0.5;
quiver(draw_x, draw_y, draw_u, draw_v);
hold on
x(k)=V*cos(theta(k-1))*T+x(k-1); % calculating x
y(k)=V*sin(theta(k-1))*T+y(k-1); % calculating y
theta(k)=atan(y(k)/x(k))+pi/2*(sign(x(k))-1); % calculating theta
