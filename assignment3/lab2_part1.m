...
[draw_x, draw_y]=meshgrid(-1:.1:1, -.5:.1:1.5);
draw_u=.5-draw_y;draw_v=draw_x;
V=0.5;
%=====================================
%% =========== The main loop ==========
while(t<=tfinal) 
...
V=(x(k)^2+(y(k)-0.5)^2)^0.5;
% vl+vr=2V
% W(vr-vl)=R
% R=0.5 W=1 
% vr=V+0.25
% vl=V-0.25
text(x(k)-0.3, y(k)+0.1, "vl: "+sprintf('%.2f', V-0.25));
text(x(k)+0.3, y(k)+0.1, "vr: "+sprintf('%.2f', V+0.25));
...
function draw_robot()
xmin=-0.9; % setting the figure limits 
xmax=0.9;
ymin=-0.4;
ymax=1.3;
