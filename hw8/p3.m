...
filePath= 'D:\Program Files\MATLAB\R2018b\toolbox\robotics\robotexamples\robotalgs\data\officemap.mat';
...
% Setup the |SensorPose|, which includes the translation along base_link's % +X, +Y direction in meters and rotation angle along base_link's +Z axis % in radians. 
...
%% Initialize AMCL Object amcl = robotics.MonteCarloLocalization; amcl.UseLidarScan = true; 
...
amcl.ParticleLimits = [500 5000]; amcl.GlobalLocalization = false; 
...
%% Localization Procedure numUpdates = 60; 
i = 0; 
while i < numUpdates 
    % Receive laser scan and odometry message. 
    scanMsg = receive(laserSub);     odompose = odomSub.LatestMessage; 
 
    % Create lidarScan object to pass to the AMCL object. 
    scan = lidarScan(scanMsg); 
 
    % For sensors that are mounted upside down, you need to reverse the     % order of scan angle readings using 'flip' function. 
    ...
    [isUpdated,estimatedPose, ~] = amcl(pose, scan); 

    % Drive robot to next pose.     wander(wanderHelper); 
